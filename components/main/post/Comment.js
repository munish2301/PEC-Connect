...
function Comment(props) {
  ...
  const getComments = async () => {
    if (props.route.params.postId !== postId || refresh) {
      ...
      const userRef = doc(postsCollectionRef, props.route.params.uid);
      const userPostsCollectionRef = collection(userRef, "userPosts");
      const postDocRef = doc(userPostsCollectionRef, props.route.params.postId);
      const commentsCollectionRef = collection(postDocRef, "comments");
      const q = query(commentsCollectionRef, orderBy("creation", "desc"));
  ...
  const onCommentSend = async () => {
    ...
    if (snapshot.exists) {
      props.sendNotification(
        snapshot.data().notificationToken,
        "New Comment",
...
const mapDispatchProps = (dispatch) =>
  bindActionCreators({ fetchUsersData, sendNotification }, dispatch);
